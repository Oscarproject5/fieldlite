...
export class TwilioService {
  ...
  async initialize(): Promise<boolean> {
    try {
      ...
      try {
        decryptedAuthToken = decrypt(data.auth_token, data.account_sid);
      } catch (decryptError) {
        console.warn('Twilio auth token decrypt failed during service init; falling back to stored value. Re-save Twilio settings to encrypt.', { tenantId: this.tenantId, error: decryptError instanceof Error ? decryptError.message : decryptError });
        decryptedAuthToken = data.auth_token;
      }
